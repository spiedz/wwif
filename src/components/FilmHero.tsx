...
const FilmHero: React.FC<FilmHeroProps> = ({ film, posterImage }) => {
  ...
  return (
    <div className={`relative mb-16 rounded-2xl overflow-hidden shadow-2xl transform transition-all duration-700 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
      ...
      <div className="h-96 sm:h-[500px] md:h-[600px] relative">
        {/* Background image */}
        <div 
          className="absolute inset-0 bg-cover bg-center transform scale-105 transition-transform duration-7000 hover:scale-110"
        ...
        {/* Content overlay */}
        <div className="absolute inset-0 flex flex-col justify-end p-8 sm:p-10 md:p-14 text-white">
          <div className="max-w-4xl relative">
            ...
            <div className={`transition-all duration-1000 delay-300 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
              ...
              <div className="flex flex-wrap gap-2 mb-5">
                {Array.isArray(film.genre) ? film.genre.map((genre: string, index: number) => (
                  <span 
                    key={`${genre}-${index}`} 
                    className="inline-block px-4 py-1.5 text-xs font-semibold bg-gradient-to-r from-primary to-primary-dark text-white rounded-full backdrop-blur-sm shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105 border border-white/10"
                  >
                    {genre}
                  </span>
                )) : (
                  <span className="inline-block px-4 py-1.5 text-xs font-semibold bg-gradient-to-r from-primary to-primary-dark text-white rounded-full backdrop-blur-sm shadow-md">
                    {film.genre}
                  </span>
                )}
              </div>
            ...
            <h1 className={`text-4xl sm:text-5xl md:text-6xl font-bold mb-5 text-white drop-shadow-xl leading-tight transition-all duration-1000 delay-500 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'}`}>
              {film.title}
            </h1>
            
            {/* Film metadata with animation */}
            <div className={`flex flex-wrap items-center text-sm md:text-base text-gray-200 mb-8 gap-x-5 gap-y-3 transition-all duration-1000 delay-700 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
              ...
              <div className="flex items-center bg-black/30 px-3 py-1.5 rounded-full backdrop-blur-sm">
                ...
                <span>{film.coordinates.length} Location{film.coordinates.length !== 1 ? 's' : ''}</span>
              </div>
            </div>
            ...
            <div className={`relative max-w-2xl transition-all duration-1000 delay-900 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'}`}>
              <p className="text-gray-100 hidden sm:block text-lg leading-relaxed shadow-lg backdrop-blur-md bg-black/20 p-6 rounded-lg border-l-4 border-primary font-medium">
                {film.description}
      ...
      <div className="block sm:hidden p-6 bg-gradient-to-b from-gray-900 to-gray-800 text-white rounded-b-2xl">
